...
class QueryEditor(Gtk.VBox):
	def __init__(self,conn_obj):
		...
		toolbar = Gtk.Toolbar()
		self.execute_btn = GtkUtil.tool_button(Gtk.STOCK_EXECUTE,messages.BTN_EXECUTE,self._on_execute)
	...
	def build_results_actions(self):
		...
		self.flush_updates_btn = GtkUtil.tool_button(Gtk.STOCK_APPLY,messages.BTN_APPLY_UPDATES,self._on_flush_updates)
		self.edit_val_btn = GtkUtil.tool_button(Gtk.STOCK_EDIT,messages.BTN_EDIT_VALUE,self._on_edit_val)
		self.add_val_btn = GtkUtil.tool_button(Gtk.STOCK_ADD,messages.BTN_ADD_ITEM,self._on_add_field)
		self.remove_val_btn = GtkUtil.tool_button(Gtk.STOCK_REMOVE,messages.BTN_REMOVE_ITEM,self._on_remove_field)
		self.copy_doc_btn = GtkUtil.tool_button(Gtk.STOCK_COPY,messages.BTN_COPY,self._on_copy_document)
		self.delete_doc_btn = GtkUtil.tool_button(Gtk.STOCK_DELETE,messages.BTN_DELETE_DOCUMENT,self._on_delete_document)
	...
	def _gen_clear_statement(self,*args): pass
	...
	def _on_execute(self, src):
		...
		def statement(db):
			...
			try:
				...
				collection = db.last_ref_collection()
				resultset = collection.last_op().result()
				
				if isinstance(resultset,pymongo.cursor.Cursor):
					resultset = CursorResultSet(resultset,collection.last_op())
				else:
					resultset = ListResultSet([resultset],collection.name,collection.last_op())
	...
	def _on_delete_document(self, src, *args):
		...
		if response == Gtk.ResponseType.OK:
			...
			def statement(db): db[collection].delete_one({'_id':doc_id})
