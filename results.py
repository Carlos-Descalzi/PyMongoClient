...
class ResultsView(Gtk.VBox):
	...
	def __init__(self):
		...
		self.first_page_btn = GtkUtil.tool_button(Gtk.STOCK_MEDIA_PREVIOUS,'First page',self._on_first_page)
		self.prev_page_btn = GtkUtil.tool_button(Gtk.STOCK_MEDIA_REWIND,'Previous page',self._on_previous_page)
		self.next_page_btn = GtkUtil.tool_button(Gtk.STOCK_MEDIA_FORWARD,'Next page',self._on_next_page)
		self.last_page_btn = GtkUtil.tool_button(Gtk.STOCK_MEDIA_NEXT,'Last page',self._on_last_page)
	...
	def _update_page_and_count(self):
		if self.resultset:
			...
			self.page_n.set_text('Page %s of %s' % (self.resultset.page + 1,self.resultset.pages))
		else:
	...
	def _on_value_edited(self, source, itr, old_value, new_value):
		...
		self.emit('update-request',self.resultset.collection, path[0],path[1:],new_value)
