...
class ConnectionEditorDialog(GladeObject):
	...
	__gsignals__ = {
		'accept' : (GObject.SIGNAL_RUN_FIRST,None, (str, object))
	}
	...
	def _valid(self):
		...
		valid = \
			self._valid_name(self.conn_name.get_text()) and \
			self._valid_db_name(self.db_name.get_text())

		#if self.conn_type.get_current_page() == 0:
		#	valid &= \
		#		self._valid_host(self.host.get_text()) and \
		#		self._valid_port(self.port.get_text())
		#else:
		#	pass

		if self.auth_type.get_current_page() == 0:
			valid &= \
				self._valid_user_name(self.auth_basic_username.get_text()) and \
				self._valid_password(self.auth_basic_password.get_text())
	...
	def _get_data(self):
		...
		if self.use_ssh_tunnel.get_active():
			tunnel = {
				...
				'password': self.tunnel_password.get_text() if self.use_tunnel_password.get_active() else None,
				'keyfile': self.tunnel_keyfile.get_filename() if not self.use_tunnel_password.get_active() else None
			}
		else:
			tunnel = None
