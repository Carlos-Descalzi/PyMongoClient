...
class ConnectionEditorDialog(GladeObject):
    ...
    def _update_actions(self, *args):
        ...
        self.tunnel_password.set_sensitive(enable_tunnel
    ...
    def _get_data(self):
        ...
        if self.use_ssh_tunnel.get_active():
            tunnel = {
                'host':
                self.tunnel_host.get_text(),
                'port':
                int(self.tunnel_port.get_text()),
                'user':
                self.tunnel_user.get_text(),
                'password':
                self.tunnel_password.get_text()
                if self.use_tunnel_password.get_active() else None,
                'keyfile':
                self.tunnel_keyfile.get_filename()
                if not self.use_tunnel_password.get_active() else None
            }
        else:
            tunnel = None
