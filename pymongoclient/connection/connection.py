...
class MongoConnection(GObject.GObject):
    ...
    def is_connecting(self):
        return self.conn_thread and self.conn_thread.is_alive() and not self.conn
    ...
    def _build_tunnel(self, conn_info):
        ...
        if tunnel_config:
            ...
            for conn in conn_info["connection"]:
                ...
                new_conn_info.append({"host": "localhost", "port": conn["port"]})
    ...
    def _build_url(self, config, conn_info):
        ...
        hostlist = ",".join([self._format_host(item, config) for item in conn_info])
